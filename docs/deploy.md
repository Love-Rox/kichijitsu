...
サーバー応答に**新しいフィールドを載せる変更**と、それに伴う**同期バックフィル世代の引き上げ**
(`apps/web/src/db/database.ts` の `CURRENT_SYNC_BACKFILL_VERSION`) を含むリリースでは、
**sync を先にデプロイしてから web をデプロイする**こと。順序を逆にすると取りこぼしが起きる。

- バックフィルは「保存済み世代 < 現行世代」の端末が起動時に `forceFull` 同期を1回走らせ、
  **成功したら現行世代を IndexedDB に記録して以後は再実行しない**仕組み
  (`App.tsx` の `runSyncBackfillIfNeeded`)。
- 世代4 (2026-07-22) はまさにこの「取りこぼしの是正」のための世代上げだった。かつては記録の条件が
  「転送が成功したか」だけで、応答に新フィールドが載っていたかを見ていなかったため、web を先に
  出すと、その窓 (旧 sync + 新 web) で起動した端末が新フィールドの**入っていない**データを取り込んだまま
  「その世代は完了」と記録し、以後バックフィルが再実行されなくなった (是正には世代をもう1つ上げるしかない)。

**2026-07-25 以降はサーバー側の対応世代を見るようになったので、順序を逆にしても取りこぼさない。**
ただし下記の運用ルールが前提なので、迷ったら従来どおり sync → web の順にしておくのが安全。

- `GET /api/me` が `syncBackfillVersion` (= sync が対応している最大世代、
  `apps/sync/src/core/google-events.ts` の `SUPPORTED_SYNC_BACKFILL_VERSION`) を返す。
- web は `min(CURRENT_SYNC_BACKFILL_VERSION, サーバーの対応世代)` までしか記録しない
  (`apps/web/src/sync/syncBackfill.ts` の `resolveEffectiveSyncBackfillVersion`)。
  旧 sync (フィールド未宣言) や異常値なら自世代へフォールバック = 従来挙動。
- **運用ルール: サーバー応答の DTO に新フィールドを足したら、`SUPPORTED_SYNC_BACKFILL_VERSION` を
  web の `CURRENT_SYNC_BACKFILL_VERSION` と一緒に上げること。** 上げ忘れると、web が新世代を
  記録できずバックフィルが毎起動で走り続ける (取りこぼしはしないが無駄な全同期が続く)。

バックフィル世代を上げない通常のリリースは順不同で構わないが、`/api`・`/auth` を早く有効に
したい場合も sync を先にするのが無難なので、常に sync → web で揃えておくとよい。
