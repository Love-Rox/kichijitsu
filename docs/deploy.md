...
- 記録の条件は「転送が成功したか」だけで、**応答に新フィールドが載っていたかは見ていない**。
  そのため web を先に出してしまうと、その窓 (旧 sync + 新 web) で起動した端末は
  新フィールドが**入っていない**データを取り込んだまま「その世代は完了」と記録してしまい、
  以後バックフィルが再実行されない (是正には世代をもう1つ上げるしかない)。
- 世代4 (2026-07-22) がまさにこの「取りこぼしの是正」のための世代上げで、同じ事故を
  繰り返さないための運用ルール (`database.ts` の世代4コメント参照)。
