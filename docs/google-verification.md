...
`calendar.events` と `tasks` について承認され、本番公開状態になった。

**承認メールに `calendar.calendarlist.readonly` が列挙されていない**点は要確認。
Google が sensitive と見なしていない (＝審査不要で通る) のか、そもそも登録が漏れて
いるのかで意味が全く違う ―― **後者なら 2026-08-06 と同じ罠**（コンソール未登録の
機密スコープを実行時に要求 → 未確認アプリ扱い → Sign In 無効化）に再び落ちる。
データアクセス画面で3スコープすべてが承認済みとして並んでいるか確認すること。

承認後の運用上の注意:
- **未確認アプリの警告画面が消える**。これまでは警告が事実上の抑止になっていたので、
  `ALLOWED_EMAILS` を空にしたまま放置すると**誰でも警告なしに連携できてしまう**。
  承認は「開放してよい」ではなく「allowlist が唯一の防壁になった」を意味する
- **同意画面の設定を変更すると再審査が必要**になる。スコープを増やす場合も同じ
...
- 本番公開後も `ALLOWED_EMAILS` は有効なまま使える（同意はできるが登録は拒否される）。
  ...
  **未設定・空文字列は「全許可」**（apps/sync/src/allowlist.ts）。落とすと黙って開放される
  ので、審査中に一時的に外した場合は**承認後すぐ戻す**こと
