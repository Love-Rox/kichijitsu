...
import { buildRenewedWatchRow, buildWatchRow, selectWatchesNeedingRenewal, type WatchRow } from '../src/core/watch-service'
...
describe('buildRenewedWatchRow', () => {
  it('keeps account/calendar/profile from the old row but swaps channel/resource/expiration', () => {
    ...
    const renewed = buildRenewedWatchRow(oldRow, 'new-channel', { resourceId: 'new-resource', expiration: 2_000 }, 1_500)
...
describe('selectWatchesNeedingRenewal', () => {
  ...
  it('selects watches expiring within the renewal window', () => {
    ...
    expect(selectWatchesNeedingRenewal(watches, NOW).map((w) => w.channel_id)).toEqual(['expiring-soon'])
  })
  ...
  it('includes watches that have already expired', () => {
    ...
    expect(selectWatchesNeedingRenewal(watches, NOW).map((w) => w.channel_id)).toEqual(['already-expired'])
  })

  it('respects a custom renewal window', () => {
    const watches = [watch({ channel_id: 'in-two-hours', expiration_ms: NOW + 2 * 60 * 60 * 1000 })]
    expect(selectWatchesNeedingRenewal(watches, NOW, 60 * 60 * 1000)).toEqual([])
    expect(selectWatchesNeedingRenewal(watches, NOW, 3 * 60 * 60 * 1000).map((w) => w.channel_id)).toEqual([
      'in-two-hours',
    ])
  })
})
