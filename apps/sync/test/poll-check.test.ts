...
describe('hasUpdatesSince', () => {
  it('returns true when Google reports at least one event updated since the watermark', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(jsonResponse(200, { items: [{ id: 'evt-1' }] }))

    const result = await hasUpdatesSince(fetchImpl, 'access-token', 'primary', '2026-07-01T00:00:00.000Z')
  ...
  it('throws GoogleApiError (does not swallow) on a non-ok response', async () => {
    ...
    await expect(hasUpdatesSince(fetchImpl, 'access-token', 'primary', '2026-07-01T00:00:00.000Z')).rejects.toThrow(
      /429/,
    )
  })
})
