...
describe("registerWatch", () => {
  it("POSTs to events.watch with the channel id/type/address/token and returns resourceId/expiration", async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse(200, { resourceId: "resource-1", expiration: "1700000000000" }),
      );
  ...
  it("returns a null expiration when Google omits it", async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(jsonResponse(200, { resourceId: "resource-1" }));
  ...
  it("throws GoogleApiError on failure (e.g. unverified/localhost address, best-effort is handled by the caller)", async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(new Response("invalid address", { status: 400 }));
...
describe("stopWatch", () => {
  it("returns true when Google accepts the stop request", async () => {
    ...
    const result = await stopWatch(fetchImpl, "access-token", {
      channelId: "channel-1",
