...
function makeTask(
  id: string,
  overrides: Partial<{ status: "needsAction" | "completed"; due: string }> = {},
) {
  return {
    id,
    title: `Task ${id}`,
    status: overrides.status ?? ("needsAction" as const),
    due: overrides.due,
  };
}
...
describe("toTaskListDTO / toGoogleTaskDTO", () => {
  it("picks id/title for task lists", () => {
    expect(toTaskListDTO({ id: "list-1", title: "Groceries" })).toEqual({
      id: "list-1",
      title: "Groceries",
    });
  });
...
describe("patchTaskStatus", () => {
  ...
  it("URL-encodes taskListId and taskId", async () => {
    ...
    expect(url).toBe(
      "https://tasks.googleapis.com/tasks/v1/lists/a%2Fb%40example.com/tasks/task%20id%20with%20spaces",
    );
  });
});
  ...
  it("combines paginated results across nextPageToken", async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse(200, { items: [makeTask("t1")], nextPageToken: "page-2" }),
      )
