...
describe('resolveLoginProfile (login モードのプロファイル解決)', () => {
  ...
  it('オーナーでログイン: 既存のオーナープロファイルへ復帰する', () => {
    const resolution = resolveLoginProfile({ profileId: 'profile-own', isOwner: true }, NEW_PROFILE_ID)
  ...
  it('接続アカウント (他人のプロファイルに is_owner=0 で属する) でログイン: ' +
    '他人の束を復活させず、自分自身の新規プロファイルを作る (今回のバグ修正の核心)', () => {
    const resolution = resolveLoginProfile({ profileId: 'someone-elses-profile', isOwner: false }, NEW_PROFILE_ID)
    expect(resolution).toEqual({ kind: 'new-profile', profileId: NEW_PROFILE_ID })
