...
describe('syncCalendar', () => {
  ...
  it('falls back to a full sync and saves a new syncToken when the old one is gone (410)', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(jsonResponse(200, { items: [makeEvent('e1')], nextSyncToken: 'fresh-sync-token' }))
  ...
  it('marks isFullSync: true when there is no stored syncToken', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(jsonResponse(200, { items: [makeEvent('e1')], nextSyncToken: 'brand-new-token' }))
  ...
  it('refreshes the access token once on 401 and retries the same page', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(jsonResponse(200, { items: [makeEvent('e1')], nextSyncToken: 'token' }))
