...
describe('createEvent', () => {
  it('POSTs events with summary and start/end dateTime+timeZone and a bearer auth header', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(new Response(JSON.stringify({ id: 'new-event' }), { status: 200 }))
...
describe('createEventWithRetry', () => {
  it('resolves with the created event id on success', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 'created-event-id' }), { status: 200 }))
  ...
  it('propagates 412/5xx as GoogleApiError instead of swallowing them', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(new Response('server error', { status: 500 }))
    const { deps } = makeDeps(fetchImpl)
