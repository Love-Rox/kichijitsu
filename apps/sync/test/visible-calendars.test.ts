...
describe('isValidVisibleCalendarsRequest', () => {
  it('accepts a valid request with a non-empty calendarIds array', () => {
    expect(isValidVisibleCalendarsRequest({ accountId: 'acc-1', calendarIds: ['cal-a', 'cal-b'] })).toBe(true)
  })
  ...
  it('rejects a non-array calendarIds', () => {
    expect(isValidVisibleCalendarsRequest({ accountId: 'acc-1', calendarIds: 'cal-a' })).toBe(false)
  })

  it('rejects calendarIds containing non-string entries', () => {
    expect(isValidVisibleCalendarsRequest({ accountId: 'acc-1', calendarIds: ['cal-a', 42] })).toBe(false)
  })
...
describe('buildCalendarPrefsRow', () => {
  it('always sets configured=1', () => {
    expect(buildCalendarPrefsRow('acc-1', 1000)).toEqual({ account_id: 'acc-1', configured: 1, updated_at: 1000 })
  })
})
