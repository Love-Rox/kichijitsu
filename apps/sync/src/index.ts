...
import { buildRenewedWatchRow, selectWatchesNeedingRenewal, type WatchRow } from './core/watch-service'
...
async function renewWatch(env: Env, watch: WatchRow): Promise<void> {
  ...
  if (!tokenResult.ok) {
    console.warn(`cron: could not get access token for account ${watch.account_id}, skipping renewal for now`)
    return
  }
  const accessToken = tokenResult.data

  if (watch.resource_id) {
    const stopped = await stopWatch(fetch, accessToken, { channelId: watch.channel_id, resourceId: watch.resource_id })
    if (!stopped) {
      console.warn(`cron: failed to stop old watch channel ${watch.channel_id} (continuing to re-register anyway)`)
    }
  }
