...
describe("monthGridRangeMs", () => {
  it("グリッド先頭日の0時からグリッド最終日の翌0時までの半開区間", () => {
    ...
    expect(toMs).toBe(
      days[41].date.add({ days: 1 }).toZonedDateTime({ timeZone: TZ }).epochMilliseconds,
    );
...
describe("bucketMonthChips", () => {
  ...
  it("時刻予定は開始日のセルにのみ割り当てる", () => {
    ...
    const groups = [
      timedGroup(
        occ({
          id: "evt-a",
          startMs: dayStartMs + 9 * 60 * 60 * 1000,
          endMs: dayStartMs + 10 * 60 * 60 * 1000,
        }),
      ),
    ];
  ...
  it("終日予定は startDate〜endDate の全セルに割り当てる", () => {
    const groups = [
      allDayGroup(
        allDayOcc({ id: "holiday-multi", startDate: "2026-07-19", endDate: "2026-07-21" }),
      ),
    ];
    const cells = bucketMonthChips(days, [], groups, TZ);
  ...
  it("終日予定を時刻予定より先に並べる", () => {
    ...
    const timed = [
      timedGroup(
        occ({ id: "evt-b", startMs: dayStartMs + 3600_000, endMs: dayStartMs + 7200_000 }),
      ),
    ];
    const allDay = [
      allDayGroup(allDayOcc({ id: "holiday-b", startDate: "2026-07-20", endDate: "2026-07-20" })),
    ];
