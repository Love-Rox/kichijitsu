...
describe("groupDuplicateOccurrences", () => {
  it("iCalUID + startMs + endMs が一致するコピーを1グループにまとめる", () => {
    const a = occ({
      id: "g:acc-1:cal-1:evt-a",
      accountId: "acc-1",
      calendarId: "cal-1",
      iCalUID: "uid-1",
    });
    const b = occ({
      id: "g:acc-2:cal-2:evt-b",
      accountId: "acc-2",
      calendarId: "cal-2",
      iCalUID: "uid-1",
    });
  ...
  it("primary はグループ内で accountId→calendarId 昇順の先頭コピー", () => {
    const later = occ({
      id: "g:acc-z:cal-1:evt-a",
      accountId: "acc-z",
      calendarId: "cal-1",
      iCalUID: "uid-1",
    });
    const earlier = occ({
      id: "g:acc-a:cal-1:evt-b",
      accountId: "acc-a",
      calendarId: "cal-1",
      iCalUID: "uid-1",
    });
  ...
  it("同じ accountId なら calendarId 昇順で primary を決める", () => {
    const b = occ({
      id: "g:acc-1:cal-b:evt-1",
      accountId: "acc-1",
      calendarId: "cal-b",
      iCalUID: "uid-1",
    });
    const a = occ({
      id: "g:acc-1:cal-a:evt-2",
      accountId: "acc-1",
      calendarId: "cal-a",
      iCalUID: "uid-1",
    });
  ...
  it("iCalUID が無い occurrence は集約せず単独グループのまま", () => {
    ...
    expect(groups).toHaveLength(2);
    expect(groups.map((g) => g.primary.id).sort()).toEqual([
      "g:acc-1:cal-1:evt-a",
      "g:acc-2:cal-2:evt-b",
    ]);
...
describe("groupDuplicateAllDayOccurrences", () => {
  it("iCalUID + startDate + endDate が一致するコピーを1グループにまとめる", () => {
    const a = allDayOcc({
      id: "g:acc-1:cal-1:holiday-a",
      accountId: "acc-1",
      calendarId: "cal-1",
      iCalUID: "uid-1",
    });
    const b = allDayOcc({
      id: "g:acc-2:cal-2:holiday-b",
      accountId: "acc-2",
      calendarId: "cal-2",
      iCalUID: "uid-1",
    });
