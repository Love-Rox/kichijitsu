...
describe("resolveEventColor", () => {
  it("calendarLookup にエントリがあればその backgroundColor を優先する", () => {
    const lookup = new Map<string, CalendarColorInfo>([
      ["acc-1:cal-1", { backgroundColor: "#ff0000" }],
    ]);
  ...
  it("accountId/calendarId が無い(ローカルイベント等)場合も target.color を返す", () => {
    ...
    expect(
      resolveEventColor(
        target({ accountId: undefined, calendarId: undefined, color: "#00ff00" }),
        lookup,
      ),
    ).toBe("#00ff00");
  });
...
describe("resolveDisplayColor", () => {
  it("hasCustomColor が true なら calendarLookup を無視して target.color をそのまま使う", () => {
    ...
    expect(resolveDisplayColor(target({ hasCustomColor: true, color: "#00ff00" }), lookup)).toBe(
      "#00ff00",
    );
  });
  ...
  it("hasCustomColor が false/undefined なら calendarLookup のカレンダー色を優先する", () => {
    ...
    expect(resolveDisplayColor(target({ hasCustomColor: false, color: "#00ff00" }), lookup)).toBe(
      "#ff0000",
    );
  ...
  it("祝日カレンダー相当のシナリオ: 初回同期でデフォルト色が焼き込まれても、後からカレンダー色を取得できれば表示はそちらに一致する", () => {
    ...
    const bakedOccurrence = target({
      hasCustomColor: false,
      color: "#3b82f6",
      calendarId: "holiday-cal",
    });
    // その後カレンダー一覧が取得できて、パネルは祝日カレンダーの本来の色 (#d50000 相当) を出す
    const lookup = new Map<string, CalendarColorInfo>([
      ["acc-1:holiday-cal", { backgroundColor: "#d50000" }],
    ]);
...
describe("resolveBusyColor", () => {
  it("解決した色が妥当ならそのまま使う", () => {
    const lookup = new Map<string, CalendarColorInfo>([
      ["acc-1:cal-1", { backgroundColor: "#ff00aa" }],
    ]);
...
describe("buildCalendarStripeColors", () => {
  it("メンバー数が上限以下ならそれぞれの解決色をそのまま順番通りに返す", () => {
    ...
    const members = [
      target({ accountId: "acc-1", calendarId: "cal-1" }),
      target({ accountId: "acc-2", calendarId: "cal-2" }),
    ];
  ...
  it("上限を超えるメンバーは先頭 (maxStripes-1) 本を実色にし、最後の1本にまとめる", () => {
    ...
    const members = Array.from({ length: 7 }, (_, i) =>
      target({ accountId: `acc-${i}`, calendarId: "cal-1", color: `#${i}${i}${i}${i}${i}${i}` }),
    );
