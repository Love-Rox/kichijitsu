...
function parseUntil(raw: string): number {
  if (/^\d{8}$/.test(raw)) {
    ...
    const instant = plainDate.toZonedDateTime("UTC").add({ days: 1 }).toInstant().epochMilliseconds;
    return instant - 1;
  }
...
function* generateCandidates(
  ...
  if (freq === "DAILY") {
    ...
    const ordinalTokens = byDay.map(parseByDayToken).filter((t) => t.ordinal !== null) as {
      ordinal: number;
      weekday: WeekdayCode;
    }[];
