...
describe('buildPendingOccurrence', () => {
  it('source:"local" の仮 occurrence を作る (書き込み先カレンダー色を使う)', () => {
    ...
    const occ = buildPendingOccurrence({ title: 'ランチ', startMs: 1_000, endMs: 4_600_000, target })
...
describe('finalizeCreatedOccurrence', () => {
  it('仮 occurrence を確定 id・source:"google" に差し替える', () => {
    ...
    const pending = buildPendingOccurrence({ title: 'ランチ', startMs: 1_000, endMs: 4_600_000, target })
