...
describe("mapGoogleEvents: workLog (hook 実績、docs/mcp.md「エージェントの作業時間記録」)", () => {
  it("extendedProperties.private.kichijitsuWorkLog='1' の単発イベントは repo+issueRef を workLog に写す", () => {
    const workLogEvent = baseEvent({
      id: "work-log-1",
      summary: "Love-Rox/kichijitsu#42",
      extendedProperties: {
        private: {
          kichijitsuWorkLog: "1",
    ...
    const result = mapGoogleEvents([workLogEvent], ctx);

    expect(result.singles[0].workLog).toEqual({ repo: "Love-Rox/kichijitsu", issueRef: "42" });
  });

  it("issueRef が無い hook 実績イベントは repo のみの workLog になる", () => {
    const workLogEvent = baseEvent({
      id: "work-log-no-issue",
      extendedProperties: {
        private: { kichijitsuWorkLog: "1", repo: "Love-Rox/kichijitsu", branch: "main" },
      },
    });

    const result = mapGoogleEvents([workLogEvent], ctx);

    expect(result.singles[0].workLog).toEqual({ repo: "Love-Rox/kichijitsu" });
  });

  it("extendedProperties が無い単発イベントは workLog: undefined のまま", () => {
    const plain = baseEvent({ id: "plain-for-worklog" });

    const result = mapGoogleEvents([plain], ctx);

    expect(result.singles[0].workLog).toBeUndefined();
  });

  it("kichijitsuWorkLog が '1' 以外の値の単発イベントは workLog: undefined のまま", () => {
    const notWorkLog = baseEvent({
      id: "not-work-log",
      extendedProperties: {
        private: { kichijitsuWorkLog: "0", repo: "Love-Rox/kichijitsu" },
      },
    });

    const result = mapGoogleEvents([notWorkLog], ctx);

    expect(result.singles[0].workLog).toBeUndefined();
  });

  it("kichijitsuWorkLog='1' でも repo が欠けていれば workLog: undefined のまま(異常値への防御)", () => {
    const missingRepo = baseEvent({
      id: "missing-repo",
      extendedProperties: { private: { kichijitsuWorkLog: "1", issueRef: "7" } },
    });

    const result = mapGoogleEvents([missingRepo], ctx);

    expect(result.singles[0].workLog).toBeUndefined();
  });

  it("hook 実績イベントは isMirror とは独立して付く(mirror ではない通常の workLog イベント)", () => {
    const workLogEvent = baseEvent({
      id: "work-log-not-mirror",
      extendedProperties: {
        private: { kichijitsuWorkLog: "1", repo: "Love-Rox/kichijitsu", issueRef: "1" },
      },
    });

    const result = mapGoogleEvents([workLogEvent], ctx);

    expect(result.singles[0].workLog).toEqual({ repo: "Love-Rox/kichijitsu", issueRef: "1" });
    expect(result.singles[0].isMirror).toBeUndefined();
  });
});
