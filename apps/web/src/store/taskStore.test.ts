...
function task(id: string, dueDate: string | null, status: TaskItem['status'] = 'needsAction'): TaskItem {
...
describe('TaskStore', () => {
  ...
  it('getRange は [fromDate, toDate] (両端 inclusive) の due 付きタスクだけを日付順で返す', () => {
    ...
    expect(store.getRange('2026-07-18', '2026-07-22').map((t) => t.id)).toEqual(['early', 'mid', 'late'])
  })
