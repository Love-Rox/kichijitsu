...
import { searchOccurrences, type SearchJumpTarget, type SearchResultItem } from '../search/searchOccurrences'
 ...
 */
export function SearchOverlay({ onClose, db, timeZone, visibleCalendarKeys, calendarLookup, onJump }: SearchOverlayProps) {
  ...
  const results = useMemo<SearchResultItem[]>(() => {
    if (!snapshot) return []
    return searchOccurrences(debouncedQuery, snapshot.occurrences, snapshot.allDay, { visibleCalendarKeys })
  ...
  function jumpTo(item: SearchResultItem) {
    onJump(
      item.kind === 'timed' ? { kind: 'timed', startMs: item.occurrence.startMs } : { kind: 'allDay', startDate: item.occurrence.startDate },
    )
    onClose()
  }
...
function SearchResultRow({ item, active, timeZone, calendarLookup, onPick, onHover }: SearchResultRowProps) {
