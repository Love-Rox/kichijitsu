 ...
 * (現値プリフィル)へ、「削除」で行ごとの2段階確認(settings/ConfirmActionControl.tsx と
 * 同じ流儀)へ切り替える。手動記録・hook 記録のどちらも編集/削除できる(agent バッジで区別)。
 * **共通部品へは畳んでいない**: 削除の状態は編集モードと同じ RowMode 一本で持っており
 * (「編集」と「削除」が同じ場所を取り合う行なので、別々の state にすると両方開ける)、
 * 成功でも失敗でも view へ戻し、失敗表示は確認ではなく「操作」欄へ出す ―― どれも
 * ConfirmActionControl の状態機械とは違う。理由の詳細はそちらの冒頭コメント(適用範囲)を参照。
