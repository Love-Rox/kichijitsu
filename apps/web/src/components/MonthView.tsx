...
import { groupDuplicateAllDayOccurrences, groupDuplicateOccurrences } from '../layout/groupDuplicates'
import {
  bucketMonthChips,
  monthGridDays,
  monthGridRangeMs,
...
import { formatAllDayDateRange, formatDetailDateTime, formatTime, WEEKDAY_LABELS } from '../layout/gridMetrics'
...
export function MonthView({
  ...
  const { fromMs, toMs } = useMemo(() => monthGridRangeMs(monthCursor, timeZone), [monthCursor, timeZone])
  const occurrences = useOccurrences(store, fromMs, toMs)
  const visibleOccurrences = useMemo(
    () => occurrences.filter((o) => o.source !== 'google' || visibleCalendarKeys.has(`${o.accountId}:${o.calendarId}`)),
    [occurrences, visibleCalendarKeys],
  )
  const groupedOccurrences = useMemo(() => groupDuplicateOccurrences(visibleOccurrences), [visibleOccurrences])
  ...
  const cells = useMemo<MonthCellData[]>(() => {
    const chipCells = bucketMonthChips(days, groupedOccurrences, groupedAllDayOccurrences, timeZone)
  ...
  if (detail && detailSubject) {
    detailDateTimeLabel = isTimedChip(detail.chip)
      ? formatDetailDateTime((detailSubject as Occurrence).startMs, (detailSubject as Occurrence).endMs, timeZone)
              ...
              return (
                ...
                >
                  <div className="month-cell-header">
                    <span className={isToday ? 'month-cell-date is-today' : 'month-cell-date'}>{cell.date.day}</span>
                  </div>
                    ...
                    {cell.visible.map((chip) => {
                      ...
                      return (
                        <button
                          ...
                          title={chip.title}
                        >
                          {isTimedChip(chip) && <span className="month-chip-time">{formatTime(chip.startMs, timeZone)}</span>}
