                ...
                >
                  ...
                  <div className="month-cell-chips">
                    {cell.visible.map((chip) => {
                      ...
                      //
                      // 2026-07-29「1日の区間として描く」は月表示には入れていない。月セルには
                      // 時間軸が無く、1日を区間に分割して描くこと自体が成立しないため
                      // (「終日の不在を終日欄に出すか」の設定を月表示に効かせないのと同じ理由)。
                      // 週表示で二重に見えていたのは「終日チップ」と「時間軸上の帯」という
                      // 別々の表現が並んだせいで、月表示では終日も時刻付きも同じ1種類のチップに
                      // 並ぶだけ ―― しかも時刻付きチップには開始時刻が付く(下記
                      // isTimedChip の .month-chip-time)ので、「自宅」「13:30 オフィス」と
                      // 読めて「その日の既定と、一部の時間帯の上書き」の関係が失われない。
                      // ここを触ると1日の全予定を一覧するという月表示の役割の方が壊れる。
