...
/**
 * GitHub 専用レーン(docs/github-integration.md フェーズ①Part B)の1日ぶんの列。
 * WeekGrid.tsx の週/day3/day1 タイムラインで、終日レーンの直下に独立した行として並ぶ。
 *
 * AllDayBar/TaskRow と違いドラッグ・詳細ポップオーバーは対象外(表示専用)。milestone は
 * 見出し的マーカー(◆ + タイトル + repo)、issue/PR はその下に小さなチップとして並べる。
 * クリックは <a target="_blank"> で GitHub 側の画面をそのまま新規タブで開く(キーボード操作・
 * 右クリックでのタブ複製など、ネイティブリンクの挙動をそのまま活かすため onClick+window.open ではなくこちらを使う)。
 */
export function GitHubLane({ groups, overflowCount }: GitHubLaneProps) {
  if (groups.length === 0 && overflowCount === 0) return null;
